...
def text_or_default(element, selector, default=None, key=None):
    """Same as one_or_default, except it returns stripped text contents of the found element
    """
    if key == 'href' and element.select_one(selector).has_attr('href'):
        return "https://www.linkedin.com" + element.select_one(selector).get('href')
...
def get_info(element, mapping, default=None):
    ...
    """
    return {key: text_or_default(element, mapping[key], default=default, key=key) for key in mapping}
...
def get_connection_info(connection):
    return get_info(connection, {
        'name': '.actor-name',
        'href': '.pv-browsemap-section__member'
    })
